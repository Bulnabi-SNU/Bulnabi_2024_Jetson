...
class VehicleController(Node):
    ...
    def __init__(self):
        ...
        self.WP7_1 = np.array([0.0, 0.0, 0.0])
    ...
    # add by chaewon. used to obstacle avoidance
    def foot_of_perpendicular(self, A, B, P):
        AP = P - A
        d = B - A
    ...
    def perpendicular_vector(self, A, B, RL):
        "Right : RL = 1, LEFT : RL = -1"
        d = B - A
        z = np.array([0.0,0.0,-1.0])
        arm = np.cross(d, z)
        perpendicular_vector = RL * arm / np.linalg.norm(arm)
    ...
    def main_timer_callback(self):
        ...
        if self.phase == 0:
            ...
            if self.vtol_vehicle_status.vehicle_vtol_state == VtolVehicleStatus.VEHICLE_VTOL_STATE_MC:
                ...
                self.publish_trajectory_setpoint(position_sp = self.current_goal)
                self.phase = 7
        elif self.phase == 7:   # obstacle avoidance
        ...
        # obstacle avoidance
            ...
            if self.obstacle_x != 'nan':
                "obstacle detected, go to the center"
                self.current_goal = self.foot_of_perpendicular(WP7, WP1, self.pos)
                self.publish_trajectory_setpoint(position_sp = self.current_goal)
                self.phase = 8.1
        elif self.phase == 8.1:
            if np.linalg.norm(self.pos - self.current_goal) < self.mc_acceptance_radius:
                "arrived to center. judge RL & evasion start"
                if self.obstacle_x >= 320:
                    "right"
                    self.arm = self.perpendicular_vector(self, self.pos, self.WP1, 1)
                else:
                    "left"
                    self.arm = self.perpendicular_vector(self, self.pos, self.WP1, -1)
                
                self.current_goal = self.current_goal + self.quoridor_radious * 0.75 * self.arm
                self.WP7_1 = self.WP7 + 0.75 * self.arm
                self.publish_trajectory_setpoint(position_sp = self.current_goal)
                self.phase = 8.2
            
        elif self.phase == 8.2:
            if np.linalg.norm(self.pos - self.current_goal) < self.mc_acceptance_radius:
                self.current_goal = self.WP7_1
                self.publish_trajectory_setpoint(position_sp = self.current_goal) 
                self.phase = 8.3
